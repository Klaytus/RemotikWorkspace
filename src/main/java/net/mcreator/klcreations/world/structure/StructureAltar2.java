...
package net.mcreator.klcreations.world.structure;
...
public class StructureAltar2 extends ElementsDimensionAdditions.ModElement {
	public StructureAltar2(ElementsDimensionAdditions instance) {
		super(instance, 28);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 650) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("klcreations", "altar2"));
				if (template == null)
					return;
				Rotation rotation = Rotation.values()[random.nextInt(3)];
				Mirror mirror = Mirror.values()[random.nextInt(2)];
				BlockPos spawnTo = new BlockPos(i, j + 68, k);
