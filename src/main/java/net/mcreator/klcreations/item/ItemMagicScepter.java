...
public class ItemMagicScepter extends ElementsDimensionAdditions.ModElement {
	...
	public void initElements() {
		...
		elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityArrowCustom.class)
				.id(new ResourceLocation("klcreations", "entitybulletmagicscepter"), ENTITYID).name("entitybulletmagicscepter").tracker(64, 1, true)
				.build());
	}
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("klcreations:magicscepter", "inventory"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		RenderingRegistry.registerEntityRenderingHandler(EntityArrowCustom.class, renderManager -> {
			return new RenderSnowball(renderManager, new ItemStack(ItemMagicProjectile.block, (int) (1)).getItem(),
		...
		public void onPlayerStoppedUsing(ItemStack itemstack, World world, EntityLivingBase entityLivingBase, int timeLeft) {
			if (!world.isRemote && entityLivingBase instanceof EntityPlayerMP) {
				...
				for (int i = 0; i < entity.inventory.mainInventory.size(); i++) {
					...
					if (stack != null && stack.getItem() == new ItemStack(ItemMagicProjectile.block, (int) (1)).getItem()
							&& stack.getMetadata() == new ItemStack(ItemMagicProjectile.block, (int) (1)).getMetadata()) {
						slotID = i;
						break;
					}
				}
				if (entity.capabilities.isCreativeMode || EnchantmentHelper.getEnchantmentLevel(Enchantments.INFINITY, itemstack) > 0
					...
					world.playSound((EntityPlayer) null, (double) x, (double) y, (double) z,
							(net.minecraft.util.SoundEvent) net.minecraft.util.SoundEvent.REGISTRY
									.getObject(new ResourceLocation(("entity.blaze.shoot"))),
							SoundCategory.NEUTRAL, 1, 1f / (itemRand.nextFloat() * 0.5f + 1f) + (power / 2));
					if (entity.capabilities.isCreativeMode) {
						entityarrow.pickupStatus = EntityArrow.PickupStatus.CREATIVE_ONLY;
					} else {
						if (new ItemStack(ItemMagicProjectile.block, (int) (1)).isItemStackDamageable()) {
							...
							entity.inventory.clearMatchingItems(new ItemStack(ItemMagicProjectile.block, (int) (1)).getItem(), -1, 1, null);
						}
					}
					if (!world.isRemote)
